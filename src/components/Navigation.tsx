...
const navigation = [
	...
	{
		name: "Discussions",
		href: "https://www.grayzonewarfare.net/forums/ammo-ballistics.14/",
	},
];
...
export default function Navigation() {
	...
	const renderLink = (item: { name: string; href: string }) => {
		...
		const className = `inline-flex items-center px-3 py-1 text-sm font-medium rounded-md ${
			current
	...
	const renderMobileLink = (item: { name: string; href: string }) => {
		const current = location.pathname === item.href;
		const className = `block w-full text-center rounded-md px-3 py-2 text-base font-medium transition-all duration-200 ${
			current
							...
							<div className="flex w-full mt-4 items-center justify-between flex-col gap-4 mb-4">
								<div className="flex flex-shrink-0 items-center mx-auto sm:mx-0">
									<div className="relative overflow-hidden rounded-full p-1 bg-accent/10 group-hover:bg-accent/20 transition-colors">
										...
										<div className="absolute inset-0 bg-accent/10 opacity-0 group-hover:opacity-100 transition-opacity rounded-full" />
									</div>
