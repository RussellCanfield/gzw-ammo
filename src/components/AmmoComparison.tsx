...
const AmmoComparison: React.FC<AmmoComparisonProps> = ({
	caliber,
	selectedTypes,
	setSelectedTypes,
}) => {
	...
	const selectedAmmoData = ammoOptions.filter((ammo) =>
		selectedTypes.includes(ammo.id),
	);
	...
	const CustomTooltip = ({ active, payload, label }: any) => {
		if (active && payload && payload.length) {
			return (
				<div
					className="bg-secondary border border-gray-700 rounded-md shadow-xl p-4"
					style={{ backgroundColor: "rgb(32, 32, 35)", opacity: 1 }}
				>
						...
						{payload.map((entry: any, index: number) => (
							...
							<div key={index} className="flex items-center">
								...
								<p className="text-sm text-text">
									...
									{entry.unit && (
										<span className="text-muted ml-1">{entry.unit}</span>
									)}
								</p>
							</div>
						))}
	...
	return (
		<div className="mt-4 pt-4 border-t border-gray-700">
			<div className="bg-primary/50 rounded-lg shadow-inner mb-6">
				<h3 className="text-lg font-semibold mb-3 text-text/90">
					Select Ammunition Types to Compare
				</h3>
				<div className="grid grid-cols-2 sm:grid-cols-3 gap-2 max-h-40 overflow-y-auto pr-2 custom-scrollbar p-4">
					{ammoOptions.map((ammo) => (
						<div key={ammo.id} className="flex items-center">
							...
							/>
							<label
								htmlFor={`ammo-${ammo.id}`}
								className="ml-2 text-sm text-text truncate"
							>
								{ammo.name}
							</label>
						</div>
								...
								<table className="min-w-full divide-y divide-gray-700 border border-gray-700 rounded-lg overflow-hidden">
									<thead className="bg-secondary">
										<tr>
											...
											<th
												colSpan={3}
												className="px-4 py-2 text-center text-xs font-medium text-muted uppercase tracking-wider border-b border-gray-700 border-l"
											>
												Helmet Armor
											</th>
											<th
										...
										<tr>
											...
											{HELMET_ARMOR_LEVELS.map((level) => (
												<th
													key={level}
													className="px-3 py-2 text-center text-xs font-medium text-muted uppercase tracking-wider border-b border-gray-700 border-l"
												>
													{level}
												</th>
											))}
										...
										{selectedAmmoData.map((ammo) => (
											...
											>
												...
												{HELMET_ARMOR_LEVELS.map((level) => (
													<td
														key={`${ammo.id}-${level}`}
														className="px-3 py-3 whitespace-nowrap text-sm text-text text-center border-r border-gray-700"
													>
														<div className="flex flex-col items-center">
															<div
																className={`h-6 w-6 rounded-full ${penetrationColorClass(ammo.helmPenetration[level])} shadow-lg`}
															/>
															<span className="text-xs text-muted mt-1">
																{penetrationLevelText(
																	ammo.helmPenetration[level],
																)}
															</span>
														</div>
													</td>
												...
												{BODY_ARMOR_LEVELS.map((level) => (
													...
													>
														<div className="flex flex-col items-center">
															<div
																className={`h-6 w-6 rounded-full ${penetrationColorClass(ammo.bodyPenetration[level])} shadow-lg`}
															/>
															<span className="text-xs text-muted mt-1">
																{penetrationLevelText(
																	ammo.bodyPenetration[level],
																)}
															</span>
														</div>
													</td>
							...
							{selectedAmmoData.map((ammo, index) => (
								...
								>
									<h4 className="font-medium text-lg mb-3 flex items-center">
										...
										<span className="text-xs bg-secondary px-2 py-0.5 rounded text-muted">
											{ammo.caliber}
										</span>
									</h4>
									<div className="grid grid-cols-2 gap-x-4 gap-y-3 text-sm">
										...
										<div className="flex items-center">
											...
											<span
												className={`font-medium ${ammo.dispersion <= 0 ? "text-green-500" : "text-yellow-500"}`}
											>
												{ammo.dispersion}%
											</span>
										</div>
												...
												(level) => {
													const penValue = HELMET_ARMOR_LEVELS.includes(
														...
														? ammo.helmPenetration[
																level as "IIA" | "IIIA" | "IIIA+"
															]
														: ammo.bodyPenetration[
																level as "IIIA" | "IIIA+" | "III" | "III+"
															];
													...
													return (
														<div key={`${ammo.id}-${level}`} className="flex-1">
															<div className="flex flex-col items-center">
																<div
																	className={`h-2 w-full ${penetrationColorClass(penValue)}`}
																/>
